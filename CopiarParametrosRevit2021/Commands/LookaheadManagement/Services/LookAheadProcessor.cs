    ...
    public class LookAheadProcessor
    {
        ...
        public void AssignWeeks(List<ScheduleData> data, string discipline)
        {
            var tasks = data.Where(t => t.MatchingRule.Disciplines
                .Contains(discipline, StringComparer.OrdinalIgnoreCase));
            ...
            foreach (var task in tasks)
            {
                ...
                foreach (var group in task.Groups)
                {
                    ...
                    foreach (var elem in candidates)
                    {
                        ...
                        if (_paramService.GetParameterValue(elem, "EJECUTADO") == "1")
                            continue;

                        if (!_filterService.ApplyFilter(elem, rule))
                            continue;

                        string elemSector = StringUtils.Clean(_paramService.GetParameterValue(elem, "SECTOR"));
                        bool match = false;

                        if (group.IsSitioLogic)
                        {
                            if (elemSector == groupSectorClean)
                                match = true;
                        }
                        else
                        {
                            string elemNivel = StringUtils.Clean(_paramService.GetParameterValue(elem, "NIVEL"));
                            if (elemSector == groupSectorClean && elemNivel == groupNivelClean)
                                match = true;
                        }
        ...
        public void ApplyExecuteAndSALogic(List<ActivityRule> rules)
        {
            ...
            {
                bool relevant = rules.Any(r =>
                    r.Categories.Contains((BuiltInCategory)elem.Category.Id.IntegerValue) &&
                    _filterService.ApplyFilter(elem, r));
